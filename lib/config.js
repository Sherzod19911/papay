...
exports.lookup_auth_member_following = (mb_id, origin) => {
     const follow_id = origin === 'follows' ? "subscribe_id" : "$_id"
    return {
        $lookup: {
            from: "follows",
            let: {
                lc_follow_id: follow_id, // Use field names or variables here
  ...
  exports.lookup_auth_member_liked = (mb_id) => {
   ...
   return {
       $lookup: {
           from: "likes",
           let: {
               lc_liken_item_id: "$_id", // Use field names or variables here
               lc_mb_id: mb_id,
               nw_my_favorite: true,
           },
           pipeline: [
               ...
               },
               {
                   $project: {
                       ...
                       mb_id: 1,
                       like_ref_id: 1,
                       my_favorite: "$$nw_my_favorite", // Fix the typo here
                   },
               },
           ],
           as: "me_liked",
